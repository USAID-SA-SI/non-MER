...
# GLOBALS ----------------------------------------------------------------------
current_month<-"2024-10" # CHANGE EACH MONTH
current_month_full<-"2024-10-31" # CHANGE EACH MONTH
last_month<- "2024-09" #CHANGE EACH MONTH
current_mo_minus_3<- "2024-07" #CHANGE EACH MONTH
lastQmo<-"2024-09" #CHANGE TO BE LAST MONTH OF MOST RECENTLY REPORTED MER Q
#last_nhls <-"2024-05" #last time we received NHLS
#last_nhls_md <- "2024-05-31"
...
df_current<-df_historic_munge %>%
  ...
  #rename_official() #comment out until set_datim is fixed
...
# 
# #read in last combined output 
# nhls_file<-list.files(monthly_extracts,pattern=paste0("_nhls"))
# nhls_final<-here("Data/monthly",nhls_file) %>% 
#             read_tsv()
# 
# #read in last combined output rejections
# vl_rejection_file<-list.files(monthly_extracts,pattern=paste0("_rejections_combined"))
# vl_rejection_final<-here("Data/monthly",vl_rejection_file)
#  
# rejections_final<-read_tsv(vl_rejection_final)
...
# 
# 
# nhls_file_historic<-list.files(monthly_extracts,pattern=paste0(last_nhls, "_nhls"))
# nhls_path_historic<-here("Data/monthly",nhls_file_historic)
# 
# nhls_h<-read_tsv(nhls_path_historic) 
# 
# 
# vl_rejection_file_historic<-list.files(monthly_extracts,pattern=paste0(last_nhls, "_rejections_combined"))
...
# write_tsv(nhls,here("Data/monthly",paste0(last_nhls,"_nhls_combined_output.txt")),na="")
# 
# 
# # REJECTIONS -------------------------------------------------------------------
# 
...
# write_tsv(rejections_combined,here("Data/monthly",paste0(last_nhls_md,"_rejections_combined_output.txt")),na="")
...
#combine -----------------------------------------------------------------------
final_df<-df_current %>% 
  filter(mon_yr >"2024-03-31") %>% #reduce non-MER to last 6 mo only
  bind_rows(targets) %>% 
  ...
  #bind_rows(nhls_final, rejections_final)
  # clean_psnu()
