...
RTC_Dispatch<-read_sheet(as_sheets_id("https://docs.google.com/spreadsheets/d/1Q737L6h3fLBOYpOekbm3pAaIE2AH8Num55JHa1o34KE/edit#gid=2074257105"), sheet = "DispatchTracker") %>% janitor::row_to_names(1) %>% mutate(`Jun2023_Facility Dispatch creation date`=as.numeric(`Jun2023_Facility Dispatch creation date`),`Jun2023_Dispatch load date`=as.numeric(`Jun2023_Dispatch load date`),
                                                                                                                                                                                                          `Jun2023_Facility last backup creation date`=as.numeric(`Jun2023_Facility last backup creation date`)) %>% 
...
ANOVA_missing<-MaTCH_Dispatch %>% select(operatingunit,fundingagency,partner,mech_code,snu,psnu,`sub-district` ,orgunit,`orgUnit Operation Status`,`Date Closed (If facility is Closed)`,`Jun2023_Facility Dispatch creation date`,`Jun2023_Dispatch load date`,`Jun2023_Facility last backup creation date` ,missing) %>% filter(missing=="Yes",`orgUnit Operation Status`!="Active - Do not submit to NDOH" & `orgUnit Operation Status`!="Closed" ) %>%  mutate(Deadline="", Status="", Partners_Comments="", Cleared_for_analytics="") 

write.xlsx(MaTCH_missing,"Dataout/ANOVA_dispatch_tracker.xlsx",sheetName="Dispatch",append=T)
